...
class LoadDetailBloc extends Bloc<LoadDetailEvents,LoadDetailState>{
  ...
  Stream<LoadDetailState> FetchLoadDetails(String weekStart, String weekEnd,int month,String year,String tractorId) async* {


    var userManager = UserManager();
    var userModel = await userManager.getData();

    var request = LoadDetailRequest(weekStart,weekEnd,month,year,tractorId);
    var body = request.toJson();
    print(body.toString());
    final _repository = Repository();
    var result = await _repository.makeLoadDetailRequest(body.toString(),userModel.token != null ? userModel.token : '');
  ...
  Stream<LoadDetailState> mapEventToState(LoadDetailEvents event)async* {
    if(event is FetchLoadDetailsEvent){
      yield* FetchLoadDetails(event.weekStart, event.weekEnd,event.month,event.year,event.tractorId);
    }
