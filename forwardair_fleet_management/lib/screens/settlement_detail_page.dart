...
class SettlementDetailsPage extends StatelessWidget {
    ...
    return Scaffold(
      ...
      body: ListView(
            ...
            _rowWidgetHolder(Constants.TEXT_DRIVER_CONTRIBUTION,
                Utils.appendDollarSymbol(settlementCheck.checkAmt), false),
            _rowWidgetHolder(Constants.TEXT_ORIGINAL_BALANCE,
                Utils.appendDollarSymbol(settlementCheck.checkAmt), false),
            _rowWidgetHolder(Constants.TEXT_DRIVER_OWING,
                Utils.appendDollarSymbol(settlementCheck.checkAmt),false),
            _rowWidgetHolder(Constants.TEXT_SERVICE_CHARGE,
                Utils.appendDollarSymbol(settlementCheck.checkAmt), false),
            _rowWidgetHolder(
                Constants.TEXT_PAYMENT,
                Utils.addDollarAfterMinusSign(
                    settlementCheck.checkAmt.toString()),false),
            _descriptionWidget(
  ...
  Widget _descriptionWidget(String title, String description, bool isLast) {
    return Container(
          margin: new EdgeInsets.only( bottom: isLast == true ? 15 : 0 , left: 15, right: 15, top: 0),
          decoration: new BoxDecoration(
            color: Colors.white,
            shape: BoxShape.rectangle,
            borderRadius:  isLast == true ? new BorderRadius.only(bottomRight: Radius.circular(5), bottomLeft: Radius.circular(5)) : new BorderRadius.all(Radius.circular(0)),
            boxShadow: <BoxShadow>[
              new BoxShadow(
  ...
  //List Tile Widget
  Widget _rowWidgetHolder(String leftSideText, String RightSideText, bool isFirst) {
    return Container(
      margin: new EdgeInsets.only( top: isFirst == true ? 15 : 0 , left: 15, right: 15),
      decoration: new BoxDecoration(
        color: Colors.white,
        shape: BoxShape.rectangle,
        borderRadius:  isFirst == true ? new BorderRadius.only(topRight: Radius.circular(5), topLeft: Radius.circular(5)) : new BorderRadius.all(Radius.circular(0)),
        boxShadow: <BoxShadow>[
          new BoxShadow(
            color: Color.fromRGBO(0, 0, 0, 0.16),
            blurRadius: 10.0,
